...
LEXICON: dict[str, dict[str, str]] = {
    "ru": {
        ...
        # Commands
        "start_new_user": "Добро пожаловать!\n\nЗдесь ты можешь встать в электронную очередь на получение документов и оплату проживания в ДАС МГУ. "
        ...
        # Registration Dialog
        "get_name_prompt": "📝 Для начала, пожалуйста, отправьте свои Фамилию, Имя и Отчество (если есть), например:\n"
        ...
        "confirm_booking_prompt": "Подтвердите запись:\n\n<b>🗓️ Дата:</b> {date} ({weekday})\n<b>🕒 Время:</b> {time}",
        "booking_successful": "✅ Вы успешно записаны!\n\n"
        "<b>{date} ({weekday}) в {time}</b> приходите в кабинет 137, окно №{window}. "
        "Лучше прийти немного заранее (за 10-20 минут).\n\n"
        "<b>С собой вам нужно будет взять:</b>\n"
        "1. Паспорт\n"
        "2. Матовую цветную фотографию 3х4\n\n"
        "<b>ВАЖНО! Если вы первокурсник или заселяетесь в ДАС впервые, перед посещением кабинета 137 вам "
        "нужно будет в порядке живой очереди пройти кабинет 141, а затем оплатить проживание в кассе (кабинет 140). "
        "Помимо вышеперечисленных документов, вам также понадобятся:</b>\n"
        ...
        "Я отправлю вам напоминание, чтобы вы точно не забыли!",
        "reschedule_successful": "✅ Ваша запись успешно перенесена!\n\n"
        "<b>Новое время: {date} ({weekday}) в {time}</b>, окно №{window}.\n\n"
        "Не забудьте все необходимые документы. Я также пришлю напоминание.",
        ...
        "<b>Время:</b> {time}\n"
        "<b>Окно:</b> №{window}\n\n"
        "<tg-spoiler>Вы можете отменить или перенести запись не позднее, чем за 3 часа до назначенного времени.</tg-spoiler>",
        ...
        "indefinitely_short": "...",
        # Admin View/Delete/Edit Exceptions
