...
def create(table, **kwargs):
    ...
    params += ")"
    values += ")"
    cursor.execute(f"INSERT INTO {table} {params} VALUES {values}", values_tuple)
...
def read(table, columns="*", fetch=-1, **kwargs):
    ...
    if kwargs:
        ...
        values += ")"
        cursor.execute(
            f"SELECT {columns} FROM {table} WHERE {params} = {values}", values_list
        )
    else:
...
def update(table, where, **kwargs):
    ...
    cursor.execute(
        f"UPDATE {table} SET {params_set} = {values_set} WHERE {params_where} = {values_where}",
...
def delete(table, **kwargs):
    ...
    params += ")"
    values += ")"
    cursor.execute(f"DELETE FROM {table} WHERE {params} = {values}", values_tuple)
