...
@router.message(CommandStart(), ~IsSigned())
async def process_start_command(message: Message):
    """
    :param message: Telegram message
    """
...
async def process_admin_command(message: Message):
    ...
    try:
        ...
        if hash == config.bot.hash:
            ...
            update(table="Users", admin=1, where=f"user_id={message.chat.id}")
        else:
...
async def process_cancel_command(message: Message, state: FSMContext):
    ...
    dt = datetime.now()
    dt_sign = datetime(
        year=2024, month=time[0], day=time[1], hour=time[2], minute=time[3]
    )
    ...
    if timedelta(0) < diff <= timedelta(hours=3):
        ...
        await message.answer(
            text=lexicon("/cancel").format(
                date=f"{str(time[0]).zfill(2)}.{str(time[1]).zfill(2)}",
...
async def process_delete_command(message: Message, state: FSMContext):
    ...
    :param state: FSM state
    """
    await message.answer(text=lexicon("/no-delete"), reply_markup=None)
...
async def process_delete_command(message: Message, state: FSMContext):
    ...
    :param state: FSM state
    """
    await message.answer(text=lexicon("/delete"), reply_markup=yesno_markup())
