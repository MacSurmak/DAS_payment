...
async def process_info_command(message: Message):
    ...
    """
    signed = len(read(table="Users", signed=1))
    ...
    while today <= last_day:
        vacant += len(
            read(table="Timetable", signed=0, month=today.month, day=today.day)
        )
...
async def process_table_command(message: Message):
    ...
    filename = (
        f"Оплата запись с {str(today.day).zfill(2)}.{str(today.month).zfill(2)} "
    ...
    with pd.ExcelWriter(f"{filename}.xlsx") as writer:
        while today <= last_day:
            ...
            day = []
            day_t = read(table="Timetable", month=today.month, day=today.day)
            ...
            if day_t:
                ...
                for i in day:
                    if i[7] is not None:
                        ...
                        surname.append(
                            read(
                                table="Users", columns="surname", user_id=i[7], fetch=1
                            )[0]
                        )

                        name.append(
                            read(table="Users", columns="name", user_id=i[7], fetch=1)[
                                0
                            ]
                        )
                        ...
                        faculty.append(
                            read(
                                table="Users", columns="faculty", user_id=i[7], fetch=1
                            )[0]
                        )

                        degree.append(
                            read(
                                table="Users", columns="degree", user_id=i[7], fetch=1
                            )[0]
                        )

                        year.append(
                            read(table="Users", columns="year", user_id=i[7], fetch=1)[
                                0
                            ]
                        )
...
async def process_open_command(message: Message):
    await message.answer(
        text=lexicon("/open"),
        reply_markup=calendar_markup_admin(datetime.today().month),
    )
...
async def aug(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text=lexicon("/open"), reply_markup=calendar_markup_admin(9)
    )
...
async def sep(callback: CallbackQuery):
    ...
    """
    await callback.message.edit_text(
        text=lexicon("/open"), reply_markup=calendar_markup_admin(10)
    )
...
async def day(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text=lexicon("extended").format(
            date=f"{str(callback.data.split('_')[2].split('-')[0]).zfill(2)}."
